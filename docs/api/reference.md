...
type BPayPaymentRequest struct {
    ...
    Passcode    string        // 4-digit Bankily verification code from the customer
}
```
...
- `Passcode`: Required, exactly 4 digits — the verification code Bankily gives the customer after they push funds to the merchant (never generated by the library)
