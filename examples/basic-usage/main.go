...
func main() {
	...
	// Step 2b: Register the provider instance for the default provider
	switch config.DefaultProvider {
	case "bpay":
		err = client.AddBPayProvider(config.Providers["bpay"])
	case "masrvi":
		err = client.AddMasrviProvider(config.Providers["masrvi"])
	}
	if err != nil {
